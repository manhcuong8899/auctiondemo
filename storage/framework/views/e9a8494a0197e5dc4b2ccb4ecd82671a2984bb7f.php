<?php $__env->startSection('content'); ?>
    ...
    <section class="content">
        <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" action="<?php echo e(url('admin/groupproducts/edit/'.$gp->id)); ?>" method="POST" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="box-body">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="name" value="<?php echo e($gp->name); ?>">
                            </div>

                            <div class="form-group">
                                <label for="title">Block hiển thị</label>
                                <select class="form-control" name="block" id="block">
                                    <option value="<?php echo e($gp->block); ?>"><?php if($gp->block=='trangchu'): ?>Trang chủ <?php else: ?> Sản phẩm <?php endif; ?></option>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control">
                                    <option disabled selected><?php echo e($gp->data); ?></option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label class="radio-inline">
                                    <input name="status" value="1"  <?php if($gp->status==1): ?> checked="checked" <?php endif; ?>  type="radio">
                                   Hiển thị</label>
                                <label class="radio-inline">
                                    <input name="status" value="0" <?php if($gp->status==0): ?> checked="checked" <?php endif; ?> type="radio">
                                    Ẩn</label>
                            </div>
...
<?php $__env->startSection('footerscripts'); ?>
    <script>
        $('#data').change(function(){
            var data = $('#data').val();
            var type = 'products';
            var couponid = '<?php echo e($gp->id); ?>';

            var nowdata = '<?php echo e($gp->data); ?>';

            if(data=="categories")
            {
                $("#renderarticle").css("display","none");
                $.ajax({
                    url: '<?php echo e(url('admin/editgroupscate')); ?>',
                    dataType: "html",
                    type: "post",
                    data: {_method: 'post', _token: '<?php echo e(csrf_token()); ?>', data: data, type: type,coupon: couponid}
                ...
                {
                    $.ajax({
                        url: '<?php echo e(url('admin/editgroupsproducts')); ?>',
                        dataType: "json",
                        type: "post",
                        data: {_method: 'post', _token: '<?php echo e(csrf_token()); ?>', pon: couponid}
                    }).done(function(jsonData) {
                        var opt = "";
